...
export default function AdminDashboard({session}: {session: any}) {
  ...
  const handleRefresh = async () => {
    ...
    const succeeded = await fetchRequests();
    ...
    if (!error) {
      ...
      if (requestRecord?.documents?.length) {
        ...
        if (documentError) {
          ...
          addToast('Anfrage gelöscht; Dokumente konnten nicht bereinigt werden', 'info');
        }
      }
    } else {
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [headers.map(escapeCsvCell).join(','), ...csvData.map((row) => row.map(escapeCsvCell).join(','))].join('\n');
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#eef5fb] text-[var(--color-ink)]">
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(rgba(28,106,168,0.025)_1px,transparent_1px),linear-gradient(90deg,rgba(28,106,168,0.025)_1px,transparent_1px)] bg-[size:48px_48px]" />
      ...
      <div className="relative z-10 flex min-h-screen flex-col gap-6 px-4 py-4 lg:flex-row lg:px-6">
        <aside className="flex w-full shrink-0 flex-col rounded-[2rem] border border-white/10 bg-[#0f172a] p-4 text-white shadow-[0_28px_70px_-42px_rgba(15,23,42,0.85)] lg:w-72 lg:p-5">
          <div className="flex items-center gap-3 rounded-[1.4rem] px-2 py-1">
            <div className="flex h-11 w-11 items-center justify-center rounded-[1rem] bg-[#1c6aa8] text-white shadow-[0_16px_28px_-18px_rgba(28,106,168,0.9)]">
              <ShieldCheck size={18} />
            </div>
            <div>
              <div className="font-heading text-xl font-semibold tracking-[-0.04em] text-white">HG Grundbesitz</div>
          ...
          <div className="mt-5 rounded-[1.6rem] border border-white/10 bg-white/[0.045] p-4">
            <div className="text-[11px] font-extrabold uppercase tracking-[0.22em] text-[#7eb4dd]">
              Interner Bereich
            </div>
          ...
          <nav className="mt-5 grid gap-2">
            <button className="flex items-center gap-3 rounded-[1.15rem] bg-white px-4 py-3 text-sm font-semibold text-[#0f172a] shadow-[0_16px_30px_-20px_rgba(255,255,255,0.7)]">
            ...
            <Link
              href="/"
              target="_blank"
              className="flex items-center gap-3 rounded-[1.15rem] border border-white/10 bg-white/[0.045] px-4 py-3 text-sm font-semibold text-slate-200 transition-colors hover:bg-white/10 hover:text-white"
            >
          ...
          <div className="mt-auto pt-6">
            <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.045] p-4">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#1c6aa8] text-sm font-semibold text-white">
                  {session.user.email?.charAt(0).toUpperCase()}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-white">{session.user.email}</p>
                  <p className="mt-1 text-[10px] font-extrabold uppercase tracking-[0.2em] text-slate-400">
                    Administrator
                  </p>
                </div>
              </div>
            ...
            <button
              ...
              className="mt-3 flex w-full items-center justify-center gap-2 rounded-[1.15rem] border border-white/10 px-4 py-3 text-sm font-semibold text-slate-300 transition-colors hover:bg-white/10 hover:text-white"
            >
              <LogOut size={16} />
              Abmelden
            </button>
...
function escapeCsvCell(value: unknown) {
  ...
  const spreadsheetSafeValue = /^[=+\-@]/.test(stringValue) ? `'${stringValue}` : stringValue;
